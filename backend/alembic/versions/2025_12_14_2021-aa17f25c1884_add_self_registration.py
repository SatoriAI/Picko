...
down_revision: Union[str, Sequence[str], None] = "48a429ba019d"
...
def upgrade() -> None:
    ...
    op.add_column(
        "event", sa.Column("registration_token", sa.String(length=64), nullable=True)
    )
    ...
    op.create_unique_constraint(
        "uq_event_registration_token", "event", ["registration_token"]
    )

    # Participant columns
    op.add_column(
        "participant", sa.Column("language", sa.String(length=5), nullable=True)
    )
    op.add_column(
        "participant", sa.Column("wishlist", sa.String(length=1000), nullable=True)
    )
