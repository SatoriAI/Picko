	...
	<main class="px-4 pb-12 sm:px-6">
		...
		<section class="py-8 sm:py-12">
			<div class="mx-auto max-w-3xl text-center">
				<h1 class="mb-4 text-4xl font-bold leading-tight tracking-tight sm:text-5xl {darkMode ? 'text-white' : 'text-slate-800'}">
					{m.tagline()}
				</h1>
				...
				<div class="flex flex-wrap justify-center gap-2.5">
					<span class="inline-flex items-center gap-1.5 rounded-full border px-3.5 py-2 text-sm font-medium shadow-sm transition-colors {darkMode ? 'border-slate-700 bg-slate-800 text-slate-300' : 'border-rose-500/15 bg-white text-slate-700'}">
		...
		<section class="py-6">
			<div class="mx-auto grid max-w-5xl gap-8 lg:grid-cols-[1fr_420px] lg:items-start lg:gap-12">
				...
				<div>
					<h2 class="mb-6 text-xl font-bold tracking-tight lg:text-2xl {darkMode ? 'text-white' : 'text-slate-800'}">
						{m.how_it_works_title()}
					</h2>
					<div class="flex flex-col gap-4">
						<article class="flex items-start gap-4 rounded-2xl border p-5 shadow-sm transition-all hover:shadow-md {darkMode ? 'border-slate-700/50 bg-slate-800/50' : 'border-slate-900/5 bg-white'}">
							<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-emerald-400 to-emerald-500 text-lg font-bold text-white shadow-md shadow-emerald-400/35">
								1
							</div>
							<div>
								<h3 class="mb-1 text-base font-semibold {darkMode ? 'text-white' : 'text-slate-800'}">{m.step1_title()}</h3>
								<p class="text-sm leading-relaxed {darkMode ? 'text-slate-400' : 'text-slate-500'}">{m.step1_desc()}</p>
							</div>
						</article>
						<article class="flex items-start gap-4 rounded-2xl border p-5 shadow-sm transition-all hover:shadow-md {darkMode ? 'border-slate-700/50 bg-slate-800/50' : 'border-slate-900/5 bg-white'}">
							<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-amber-400 to-orange-500 text-lg font-bold text-white shadow-md shadow-orange-400/35">
								2
							</div>
							<div>
								<h3 class="mb-1 text-base font-semibold {darkMode ? 'text-white' : 'text-slate-800'}">{m.step2_title()}</h3>
								<p class="text-sm leading-relaxed {darkMode ? 'text-slate-400' : 'text-slate-500'}">{m.step2_desc()}</p>
							</div>
						</article>
						<article class="flex items-start gap-4 rounded-2xl border p-5 shadow-sm transition-all hover:shadow-md {darkMode ? 'border-slate-700/50 bg-slate-800/50' : 'border-slate-900/5 bg-white'}">
							<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-rose-400 to-rose-500 text-lg font-bold text-white shadow-md shadow-rose-400/35">
								3
							</div>
							<div>
								<h3 class="mb-1 text-base font-semibold {darkMode ? 'text-white' : 'text-slate-800'}">{m.step3_title()}</h3>
								<p class="text-sm leading-relaxed {darkMode ? 'text-slate-400' : 'text-slate-500'}">{m.step3_desc()}</p>
							</div>
						</article>
					</div>
				</div>
				...
				<!-- Form Card (on right) -->
				<div class="rounded-2xl border p-6 shadow-lg transition-colors sm:p-8 {darkMode ? 'border-slate-700/50 bg-slate-800/80 shadow-slate-900/50' : 'border-slate-900/5 bg-white shadow-slate-900/5'}">
					<h2 class="mb-6 text-center text-xl font-semibold {darkMode ? 'text-white' : 'text-slate-800'}">
						{m.form_title()}
					</h2>
					<form
						onsubmit={(e: Event) => {
					...
					>
						<div class="mb-4">
							<label for="eventName" class="mb-1.5 block text-sm font-medium {darkMode ? 'text-slate-300' : 'text-slate-600'}">
								{m.label_event_name()}
							</label>
							<input
								type="text"
						...
						<div class="mb-4">
							<label for="maxAmount" class="mb-1.5 block text-sm font-medium {darkMode ? 'text-slate-300' : 'text-slate-600'}">
								{m.label_max_amount()}
							</label>
							<div class="flex">
								<input
								...
								/>
								<span class="flex items-center rounded-r-xl border-[1.5px] border-l-0 px-4 text-sm font-medium {darkMode ? 'border-slate-600 bg-slate-700 text-slate-400' : 'border-slate-200 bg-slate-100 text-slate-500'}">
									{m.currency_suffix()}
								</span>
							</div>
						</div>
						...
						<div class="mb-4">
							<label for="eventDate" class="mb-1.5 block text-sm font-medium {darkMode ? 'text-slate-300' : 'text-slate-600'}">
								{m.label_date()} <span class="font-normal {darkMode ? 'text-slate-500' : 'text-slate-400'}">{m.label_date_optional()}</span>
							</label>
							<input
								type="date"
								id="eventDate"
								bind:value={eventDate}
								class="w-full rounded-xl border-[1.5px] px-4 py-3 text-base transition-all focus:border-rose-500 focus:ring-[3px] focus:ring-rose-500/10 focus:outline-none {darkMode ? 'border-slate-600 bg-slate-700/50 text-white focus:bg-slate-700' : 'border-slate-200 bg-slate-50 text-slate-800 focus:bg-white'}"
							/>
						</div>

						<div class="mb-4">
							<label for="participants" class="mb-1.5 block text-sm font-medium {darkMode ? 'text-slate-300' : 'text-slate-600'}">
					...
					<div class="my-5 flex items-center gap-4 text-sm {darkMode ? 'text-slate-500' : 'text-slate-400'}">
					...
					<button
						type="button"
						class="w-full cursor-pointer rounded-xl border-[1.5px] bg-transparent px-6 py-3 text-base font-medium transition-all hover:border-rose-500 hover:bg-rose-500/5 hover:text-rose-500 {darkMode ? 'border-slate-600 text-slate-400' : 'border-slate-200 text-slate-600'}"
						onclick={handleDemo}
					>
						{m.button_demo()}
					</button>
