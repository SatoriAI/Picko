<script lang="ts">
	...
	import WishlistDisplay from './WishlistDisplay.svelte';

	interface Props {
		/**
		 * Name of the person viewing this reveal (the giver).
		 */
		giverName: string;
		/**
		 * Name of the person to gift (the receiver).
		 */
		receiverName: string;
		/**
		 * Receiver's wishlist (comma-separated).
		 */
		receiverWishlist: string | null;
		/**
		 * Event name for the badge.
		 */
		eventName: string;
		/**
		 * Event date (optional).
	...
	<div class="relative w-full max-w-md">
		...
		{#if !isRevealed}
			...
			<div class="flex flex-col items-center">
				...
				<!-- Greeting -->
				<h1 class="mb-3 text-center text-2xl font-bold text-slate-800 sm:text-3xl dark:text-white">
					{m.join_greeting({ name: giverName })}
				</h1>
		...
		{:else}
			...
			<div class="flex flex-col items-center">
				...
				<Card class="w-full animate-scale-in p-8 text-center">
					...
					<div class="space-y-3 text-sm">
						{#if eventDate}
							...
							>
								...
								<span>
									{m.join_event_date()}:
									<strong>{formatDateLong(eventDate, getLocale())}</strong>
								</span>
							</div>
						{/if}
						{#if maxAmount}
							...
							>
								...
								<span>
									{m.join_budget()}:
									<strong>{maxAmount} {currency ?? DEFAULT_CURRENCY}</strong>
								</span>
							</div>
						{/if}
					</div>
					...
					{#if receiverWishlist}
						<div class="mt-6">
							<WishlistDisplay wishlist={receiverWishlist} />
						</div>
					{/if}
				</Card>
